...
function updateStudent($studentInput)
{
    ...
    if (empty(trim($name))) {
        ...
        return error422('Enter your contact information');
    } elseif (empty(trim($class))) {
        return error422('Enter your class');
    } else {
        $sql = "UPDATE student SET name = ?, class = ? ,date_of_birth = ?, address = ?, contact_info = ? WHERE id = ? LIMIT 1";
        $stmt = $connection->prepare($sql);
        $stmt->bind_param("sssssi", $name, $class,$date_of_birth, $address, $contact_info, $studentId);
